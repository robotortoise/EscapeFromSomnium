...
namespace Naninovel
{
    ...
    public static class Engine
    {
        ...
        public static async UniTask InitializeAsync (EngineParams @params)
        {
            ...
            objects.Clear();
            services.Clear();
            services.AddRange(@params.Services);

            for (var i = 0; i < services.Count; i++)
            {
                OnInitializationProgress?.Invoke(.25f + .5f * (i / (float)services.Count));
                await services[i].InitializeServiceAsync();
        ...
        /// <param name="spot">When provided, will prepend associated script name and line index to the message.</param>
        public static void Log (string message, PlaybackSpot? spot = null) => logger.Log(BuildLogMessage(message, spot));

        /// <summary>
        /// Logs a warning with the current logger.
        /// </summary>
        /// <inheritdoc cref="Log"/>
        public static void Warn (string message, PlaybackSpot? spot = null) => logger.Warn(BuildLogMessage(message, spot));

        /// <summary>
        /// Logs an error message with the current logger.
        /// </summary>
        /// <inheritdoc cref="Log"/>
        public static void Err (string message, PlaybackSpot? spot = null) => logger.Err(BuildLogMessage(message, spot));
        ...
        private static string BuildLogMessage (string message, PlaybackSpot? spot)
        {
            ...
            return $"Naninovel script `{spot.Value.ScriptName}` at line #{line}: {message}";
        }
    }
}
